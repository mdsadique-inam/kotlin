...
internal inline fun <reified T : ConstantValue<*>> FirExpression.toConstantValue(
    ...
    val valueFromFir = when (this) {
        ...
        else -> this.toConstantValue(session)
    }
...
private fun FirElement.toConstantValue(session: FirSession): ConstantValue<*>? {
    return when (this) {
        ...
        is FirQualifiedAccessExpression -> {
            when (val symbol = this.toResolvedCallableSymbol()) {
                ...
                is FirConstructorSymbol -> {
                    ...
                    val mappingToConstantValues = mappingToFirExpression.mapValues { it.value.toConstantValue(session) ?: return null }
        ...
        is FirAnnotation -> {
            ...
            val mappingToConstantValues = mappingToFirExpression.mapValues { it.value.toConstantValue(session) ?: return null }
            this.toAnnotationValue(mappingToConstantValues, session)
        }
        ...
        is FirArrayLiteral -> ArrayValue(this.argumentList.arguments.mapNotNull { it.toConstantValue(session) })
        is FirVarargArgumentsExpression -> {
            ...
            return ArrayValue(arguments.mapNotNull { it.toConstantValue(session) })
        }
        else -> null
    }
}
...
private fun FirAnnotation.evaluateToAnnotationValue(
    ...
    val result = argumentMapping.mapping.mapValuesTo(mutableMapOf()) { (name, _) ->
        mappingFromFrontend[name]?.let {
            ...
            evaluatedValue?.toConstantValue(session)
    ...
    return this.toAnnotationValue(result as Map<Name, ConstantValue<*>>, session)
}

fun FirAnnotation.toAnnotationValue(mapping: Map<Name, ConstantValue<*>>, session: FirSession): AnnotationValue {
